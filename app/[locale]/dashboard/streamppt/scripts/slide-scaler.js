...
(function() {
    ...
    rafCallback(() => {
      try {
        ...
        if (contentWidth < config.minContentWidth || contentHeight < config.minContentHeight) {
          console.log('内容尺寸不合理，稍后重试', contentWidth, contentHeight);
          state.isScaling = false;
          return;
        }
  ...
  function handleResize(width, height) {
    if (!width || !height || width < 10 || height < 10) {
      console.log('无效的尺寸', width, height);
      return;
    }
    
    console.log('处理尺寸变化', width, height);
  ...
  function attemptRescale() {
    if (state.scaleAttempts < config.maxScaleAttempts) {
      state.scaleAttempts++;
      console.log(`尝试重新缩放 ${state.scaleAttempts}/${config.maxScaleAttempts}`);
    ...
    try {
      ...
      rafCallback(() => {
        ...
        if (pptAnimations.length > 0) {
          console.log(`找到${pptAnimations.length}个PPT动画元素`);
