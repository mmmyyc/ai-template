...
2. 内容处理规则：
   ...
   - 空间控制：严格遵守1280px宽度限制，高度上允许小部分溢出，确保所有核心内容完整显示
...
8. 风格一致性规则：
   - 历史参考：如果用户提供了之前生成的HTML代码，在保证自身不会出现问题的条件下，必须分析其风格特征并在新幻灯片中保持一致
   - 色彩方案延续：在保证本身颜色色彩分明的前提下，复用之前幻灯片中的主色、辅助色和强调色系统，确保视觉语言一致
   - 布局模式保持：在保证本身布局模式清晰的前提下，保持与历史幻灯片相似的布局结构、空间分配和元素排列方式
   - 组件风格同步：在保证本身组件风格的不会出错前提下，选用与历史幻灯片相同或风格相近的组件，维持设计语言一致性
   - 动画效果协调：在保证本身动画效果的不会出错前提下，保持与历史幻灯片相似的动画类型、时长和触发方式
   - 字体系统统一：在保证本身字体系统的不会出错前提下，确保字体选择、大小层级和样式处理与历史幻灯片一致
...
- 注意!可能页面出现无限高度拉伸，最可能的原因是CSS中 h-full (即 height: 100%) 类与 Chart.js 响应式图表行为之间的冲突,解决方案**移除或调整图表容器的 h-full，让图表的高度由 canvas 标签的 height 属性或一个固定的包装器 div 高度来决定，而不是百分比**
示例：
\`\`\`html
...
- 阶段2 - 幻灯片制作:
  ...
  - 步骤 2: 如果用户提供了历史HTML代码，分析其设计风格、布局结构、色彩系统和组件使用模式，保证主标题的位置不要改变
  - 步骤 3: 根据内容特性，选择最合适的布局模式（经典标题-内容、网格卡片、中心焦点、分屏比较或时间线等），确保内容表达最为清晰有效
  - 步骤 4: 应用规整的网格系统进行布局设计，确保元素对齐、比例协调、视觉平衡，创建专业美感
  - 步骤 5: 理解并应用用户指定的设计风格，同时保持与历史HTML一致的视觉语言
  - 步骤 6: 严格评估内容是否真正需要图表展示，仅当内容包含明确需要可视化的数据或关系时，才确定引入Chart.js或Mermaid.js
  - 步骤 7: 若确实需要图表，根据数据特性和内容需求选择最合适且最简洁的图表类型，确保图表尺寸合适，不占用过多空间
  - 步骤 8: 构建完整的HTML文档结构，仅在需要时引入图表相关的CDN资源，包含所有必要的HTML标签
  - 步骤 9: 应用紧凑的布局设计，优化元素间距和尺寸，确保核心内容在1280px宽度内完整显示，高度允许适当延展
  - 步骤 10: 添加适当的动画效果，确保视觉层次清晰且不分散注意力，动画应该引导观众关注核心内容
  - 步骤 11: 检查HTML结构完整性、内容完整性、样式一致性和技术规范遵循情况，确保没有内容被设计元素遮挡或削弱
  - 预期结果: 生成一个视觉吸引人、结构清晰且布局规整的单页HTML幻灯片，内容传达清晰且完整，能在1280px宽度内完整展示，核心内容无需滚动即可查看，并与之前生成的幻灯片保持风格一致
